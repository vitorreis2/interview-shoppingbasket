...
public class BasketTest {
    ...
    void consolidateBasketTest() {
        Basket basket = new Basket();
        basket.add("P001", "Product 1", 3);
        basket.add("P002", "Product 2", 3);
        basket.add("P001", "Product 1", 5);

        basket.consolidateItems();

        assertEquals(2, basket.getItems().size());
        assertEquals("Product 1", basket.getItems().get(0).getProductName());
        assertEquals(8, basket.getItems().get(0).getQuantity());
        assertEquals("Product 2", basket.getItems().get(1).getProductName());
        assertEquals(3, basket.getItems().get(1).getQuantity());
